			...
			if (valPass.val().length > 0) {
				...
				if (valPass.val().length < 4) {
					if ($('#user_password_first_error').hasClass('error') == false) {
						valPass.after(
							'<label id="user_password_first_error" class="error" for="user_password_first" style="display: block;">Mật khẩu phải chứa 4-20 ký tự</label>'
						)
				...
				$('#user_password_first_error').remove()
				valPass.after(
					'<label id="user_password_first_error" class="error" for="user_password_first" style="display: block;">Vui lòng nhập mật khẩu.</label>'
				)
				valPass.addClass('error')
			}
		}
...
function checkvali(id = '') {
	...
	if (ccpass.val() == '') {
		...
		if (ccpass.val().length < 4) {
			if ($('#user_password_first_error').hasClass('error') == false) {
				ccpass.after(
					'<label id="user_password_first_error" class="error" for="user_password_first" style="display: block;">Mật khẩu phải từ 4-20 ký tự</label>'
				)
				ccpass.addClass('error')
				ccpass.focus()
				returnform = false
